...
module glacier_boundary_mod
  ...
  type, public :: glacier_boundary
    ...
    ! [[glacier_boundary(type):get_boundaries_residual]] to return an array
    ! with the residuals representing deviation from satisfying the
    ! conditions. This can then be appended to a [[glacier(type)]]'s residual 
...
contains
  ...
  function residual_size(this)
    ...
    !  Date: September 2016
    !
    ! Default implementation of the [[glacier_boundary(type):residual_size]]
    ! routine providing the size of the residual array returned by
    ! [[glacier_boundary(type):boundary_residuals]]. For performance
